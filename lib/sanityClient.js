...
export async function getCategories() {
  return await client.fetch(`*[_type == "category" && !(_id in path('drafts.**'))] {
    _id,
    title,
    description
  }`);
...
export async function getFeaturedPosts() {
  return await client.fetch(`*[_type == "post" && featured == true && !(_id in path('drafts.**'))] | order(publishedAt desc)[0...3] {
    _id,
    title,
    slug,
    mainImage,
